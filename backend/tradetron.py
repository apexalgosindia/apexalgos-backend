...
def calculate_pnl(strategies: list) -> dict:
    ...
    def add_shared_strategy(self, shared_code: str) -> str:
        """
        Add a shared strategy to the master account using shared code.
        Returns the SID string on success, empty string on failure.
        """
        if not self.logged_in:
            if not self.login():
                return ""
        xsrf = self._xsrf()
        try:
            # Tradetron's subscribe/add shared strategy endpoint
            url     = f"{BASE_URL}/api/strategies/subscribe"
            payload = json.dumps({"shared_code": shared_code}).encode()
            req     = urllib.request.Request(
                url, data=payload,
                headers={"User-Agent": "Mozilla/5.0",
            ...
            sid = (data.get("data") or {}).get("id") or data.get("id") or data.get("sid") or ""
            return str(sid)
        except Exception as e:
            log.error(f"add_shared_strategy error: {e}")
            return ""
